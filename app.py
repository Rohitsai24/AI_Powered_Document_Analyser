...
def main():
    st.set_page_config("Document Analyzer")
    st.header("AI Powered Document Analyser")
    ...
    with st.sidebar:
        st.title("Menu:")
        pdf_path = st.file_uploader("Upload your PDF File and Click on the Submit & Process Button", type=["pdf"])
        if pdf_path is not None:
            with st.spinner("Processing..."):
                raw_text = get_pdf_text([pdf_path])
                raw_text += extract_text_from_images(pdf_path)
